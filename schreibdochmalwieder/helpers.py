...
def name_to_number(name, n_max):
    """
    Returns a (default: 2-digit) number for a given string. By using a hash function,
    it is ensured, that strings with a low Levenshtein distance yield completely
    different numbers, yet the same string always yields the same.
...
def map_range(n, n_exp, n_max):
    """
    Maps a n digit number to another n digit number, where n_max is the maximum number
    for n digit numbers.
